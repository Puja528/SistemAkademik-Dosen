...
export default function DashboardUtama() {
  ...
  return (
    <div className="p-6 flex flex-col gap-5 bg-gray-50/50 min-h-screen animate-fadeIn font-sans">
      ...
      {/* 1. SEKSI HEADER PORTAL (BULATAN DAN TEKS SIAKAD TELAH DIHAPUS) */}
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: "IPK Kumulatif", value: dataAkademik.profil ? Number(dataAkademik.profil.ipk).toFixed(2) : "0.00", sub: "Akumulasi Keseluruhan" },
          { label: "IP Semester (IPS)", value: activeTab === 'khs' ? khsPerSemester.ips : 'N/A', sub: `Nilai ${selectedSemester}` },
          { label: "SKS Selesai", value: `${dataAkademik.nilai.reduce((acc, curr) => acc + (curr.jadwal?.sks || 0), 0)} SKS`, sub: "Total Kredit Lulus" },
                ...
                <tbody>
                  {khsPerSemester.nilai.map((n) => (
                    <tr key={n.id} className="hover:bg-gray-50/70 transition-colors">
                      ...
                      <TD className="text-center font-bold text-slate-600">{n.jadwal?.sks}</TD>
                ...
                <tbody>
                  {transkripSemua.map((t) => (
                    <tr key={t.id} className="hover:bg-gray-50/70 transition-colors">
                      ...
                      <TD className="text-center font-black text-[#1a3a6b] font-mono uppercase">{t.grade}</TD>
                    </tr>
                  ))}
                  {transkripSemua.length === 0 && (
                    <tr>
                      <td colSpan="3" className="px-3 py-10 text-center text-gray-400 text-[12px] font-medium">
