...
const TH = ({ children, className = "" }) => (
  <th className={`text-[11px] font-bold text-slate-500 uppercase tracking-wider px-3 py-3 border-b border-gray-200 text-left ${className}`}>{children}</th>
);

const TD = ({ children, className = "" }) => (
  <td className={`px-3 py-3 text-xs text-slate-600 border-b border-gray-100 transition-colors ${className}`}>{children}</td>
);
  ...
  return (
    <div className="p-6 flex flex-col gap-6 bg-[#f4f6f9] min-h-screen font-sans text-xs text-slate-700 w-full">
      ...
      <div className="rounded-xl p-5 text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm" style={{ background: "linear-gradient(135deg, #1a3a6b 0%, #244b86 60%, #2e5fa3 100%)" }}>
        <div>
          <h1 className="text-lg font-black m-0 mb-1 tracking-tight">Selamat Datang, {profilDosen?.nama}</h1>
          <p className="text-xs opacity-85 m-0 font-medium">NIDN: {profilDosen?.nidn} · Program Studi {profilDosen?.program_studi}</p>
        </div>
        <div className="text-left sm:text-right flex-shrink-0 bg-white/10 backdrop-blur-sm px-4 py-2 rounded-xl border border-white/10 w-full sm:w-auto">
          <div className="text-[10px] opacity-75 mb-0.5 font-bold uppercase tracking-wider">{hari}</div>
          <div className="text-lg font-mono font-black tracking-wider">{jam} WIB</div>
        </div>
      </div>
        ...
        {[
          ...
          <div key={i} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm flex justify-between items-center">
            <div className="space-y-1">
              ...
              <span className="text-[11px] text-gray-400 font-medium block">{sub}</span>
            </div>
            <div className={`p-3.5 rounded-xl ${bgIcon}`}><Icon className="text-xl" /></div>
          </div>
        ))}
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm flex flex-col">
          ...
          <div className="overflow-x-auto flex-1">
            <table className="w-full border-collapse">
              <thead>
                <tr>{["Jam", "Mata Kuliah", "Ruang"].map((h) => <TH key={h}>{h}</TH>)}</tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {JADWAL_HARI_INI.length > 0 ? (
                  JADWAL_HARI_INI.map((j, i) => (
                    <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-3 py-3 text-xs text-slate-900 font-bold font-mono whitespace-nowrap">{j.jam}</td>
                      <td className="px-3 py-3 text-xs">
                        <div className="font-bold text-slate-800 uppercase">{j.matkul}</div>
                        <div className="text-[11px] text-slate-400 font-medium">Kelas {j.kelas}</div>
                      </td>
                      <td className="px-3 py-3 text-xs text-slate-700 font-semibold">{j.ruang}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan="3" className="text-center py-8 text-gray-400 font-medium">Tidak ada jadwal mengajar untuk hari ini.</td>
                  </tr>
                )}
              </tbody>
            </table>
            ...
            <table className="w-full border-collapse">
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredNilai.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan="2" className="text-center py-8 text-gray-400 font-medium">Data rekapitulasi nilai kosong.</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <span className="text-sm font-bold text-slate-950">Rekapitulasi Kehadiran Mahasiswa</span>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-50">
                {["NIM / ID", "Nama Mahasiswa", "Kehadiran", "Persentase Grafis"].map(h => <TH key={h}>{h}</TH>)}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {filteredAbsensi.length > 0 ? (
                filteredAbsensi.map((r, i) => (
                  <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                    <TD className="font-mono font-bold text-slate-900 tracking-wide">{r.nim}</TD>
                    <TD className="font-bold text-slate-800 uppercase">{r.nama}</TD>
                ...
                <tr>
                  <td colSpan="4" className="text-center py-10 text-gray-400 font-medium">Mahasiswa tidak ditemukan.</td>
                </tr>
              )}
            </tbody>
          </table>
